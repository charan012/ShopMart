...
const PasswordUpdateForm = () => {
  ...
  useEffect(() => {
    ...
    if (updata.new_password && updata.confirm_password) {
      setPasswordMismatchError(
        updata.new_password !== updata.confirm_password ? 'Passwords do not match.' : ''
      );
    } else {
      setPasswordMismatchError('');
    }
  }, [updata.new_password, updata.confirm_password]);

  const handleChange = (e) => {
    const { name, value } = e.target;
    setUpdata((prevState) => ({ ...prevState, [name]: value }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        email: data.email,
        current_password: updata.current_password,
        new_password: updata.new_password,
        confirm_password: updata.confirm_password,
      };

      const response = await axios.post('https://technxt.in/demo_api/forgot_password.php', payload);
                ...
                <div>
                  <div className="mb-lg-9 mb-5">
                    ...
                    {passwordMismatchError && (
                      <div style={{ color: 'red', marginBottom: '10px' }}>{passwordMismatchError}</div>
                    )}
                    <form onSubmit={handleSubmit}>
                      <div className="row g-3">
                        ...
                        <PasswordField
                          ...
                          value={updata.confirm_password}
                          onChange={handleChange}
                          showPassword={showConfirmPassword}
                          setShowPassword={setShowConfirmPassword}
                          disabled={!updata.new_password}
                        />
                       {/* Submit Button */}
                        <button
                          type="submit"
                        ...
                        <Link to="/Dashindex" className="btn btn-light">Back</Link>
                      </div>
                    </form>
                  </div>
                </div>
              ...
              <div className="row g-4">
                ...
                <div className="col-6 col-sm-6 col-md-3">
                  ...
                  <ul className="nav flex-column">
                    ...
                    <li className="nav-item mb-2"><Link to="/Shopcheck" className="nav-link">Shop Checkout</Link></li>
                  </ul>
                </div>
                <div className="col-6 col-sm-6 col-md-3">
...
const PasswordField = ({ label, name, value, onChange, showPassword, setShowPassword, disabled }) => (
